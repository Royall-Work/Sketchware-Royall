...
import Araaf.Royall.events.EventHelper;
...
*/        
        public static String[] getActivityEvents() {
                ...
                for (EventHelper.EventData event : EventHelper.EVENTS) {
                        if (event != null && event.var.isEmpty()) {
...
*/        
        public static void addEvents(Gx gx, ArrayList<String> list) {
                ...
                for (EventHelper.EventData event : EventHelper.EVENTS) {
                        if (event != null && !event.var.isEmpty() && gx.a(event.var)) {
                                list.add(event.name);
                        }
                }
...
*/        
        public static void addListeners(Gx gx, ArrayList<String> list) {
                ...
                for (EventHelper.EventData event : EventHelper.EVENTS) {
                        if (event != null && !event.listener.isEmpty() && gx.a(event.var)) {
                                if (!list.contains(event.listener)) {
...
*/        
        public static void addEventsToListener(String name, ArrayList<String> list) {
                switch (name) {
                        ...
                        default:
                        for (EventHelper.EventData event : EventHelper.EVENTS) {
                                if (event != null && name.equals(event.listener)) {
                                        list.add(event.name);
                                }
                        }
        ...
        public static int getIcon(String name) {
                return switch (name) {
                        ...
                        default -> {
                                EventHelper.EventData helperEvent = findHelperEvent(name);
                                if (helperEvent != null) {
                                        yield helperEvent.icon;
                                }
        ...
        public static String getEventCode(String targetId, String name, String param) {
                return switch (name) {
                        ...
                        default -> {
                                EventHelper.EventData helperEvent = findHelperEvent(name);
                                if (helperEvent != null) {
                                        String formattedCode = helperEvent.code
                                        .replace("###", targetId)
        ...
        public static String getBlocks(String name) {
                return switch (name) {
                        case "Import", "initializeLogic", "onSwipeRefreshLayout", "onPreExecute" -> "";
                        case " onLongClick" -> "%s.###_getText";
        ...
        public static String getSpec(String name, String event) {
                return switch (event) {
                        ...
                        default -> {
                                EventHelper.EventData helperEvent = findHelperEvent(event);
                                if (helperEvent != null) {
                                        yield helperEvent.spec.replace("###", name);
                                }
                                for (int i = 0, cachedCustomEventsSize = cachedCustomEvents.size(); i < cachedCustomEventsSize; i++) {
                                        HashMap<String, Object> customEvent = cachedCustomEvents.get(i);
                                        if (customEvent != null) {
        ...
        private static EventHelper.EventData findHelperEvent(String name) {
                for (EventHelper.EventData event : EventHelper.EVENTS) {
